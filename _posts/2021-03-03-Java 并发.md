...
```
synchronized 
1 实现：通过jvm enter exit+java对象内存标志位(线程id+进入次数)
2 作用场景：1 实例方法，锁住实例对象，2 静态方法，锁住class 对象，即所有对象 3 代码块，锁给定的对象

单例模式
1 写法 2 volatile 为啥要用 -> 创建大致分为3部，分配单例内存空间，对象初始化，把对象指向引用，防止指令重排序
...
lock&&sync区别
1 都可重入
2 synv基于jvm指令 lock基于jdkapi
3 lock可尝试可中断 但是必须释放锁，基于条件锁（1.5）

volatile

功能：可见性，有序性，单次读写原子性。

缺点：不能保证多次读写原子，比如i++,本质是一次读 一次写

1实现：可见性(缓存一致性)  有序性 hapend-before  内存屏障
2 缺点不能保证原子性，优点轻量级，cpu开销小
```
...
```
https://www.cnblogs.com/paddix/p/5428507.html   关于 volatile的文章
...
```
不可尝试，非公平，不可中断，异常自动释放，性能跟资源竞争程度成正比

可尝试，可重入，可中断，异常不自动释放，必须在finally释放，性能跟资源竞争程度无关
```
