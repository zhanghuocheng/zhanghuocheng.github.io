...
```
https://www.cnblogs.com/paddix/p/5428507.html  锁相关文章
```
...
```
参考文章 http://ifeve.com/syn-jmm/   -Doug Lea 
重排序 编译器 处理器 内存系统
```
...
``` 
1 功能：可见性，有序性，单次读写原子性(不保证多次读写原子性比如i++ )
2 原理 
可见性 L1 缓存和主缓存保持一致性  MESI 协议 
有序性 防止编译和处理器的指令中排序  -hapend-before-内存屏障
3 优缺点 ：不能保证多次读写的原子性，轻量级


```

### 屏障-内存屏障解决的只是顺序一致性的问题

```
背景 ：1 编译器优化乱序和CPU执行乱序的问题可以分别使用优化屏障 (Optimization Barrier)和内存屏障 (Memory Barrier)这两个机制来解决：2 定义：内存屏障是硬件之上、操作系统或JVM之下，是一种标准，各厂商可能采用不同的实现。
...
### 缓存一致性协议-实现 1 MESI协议。Intel  2 AMD使用MOESI

```
功能:保证cpu L1L2和L3缓存之间的可见性 
```
...
1 功能-锁
2 原理基于aqs的锁 aqs？
3 优缺点 可尝试，可重入，可中断，异常不自动释放，必须在finally释放，性能跟资源竞争程度无关 基于条件锁（1.5
```
...
下面我以一个常见的考官题为例讲解一下 synchronized 关键字的具体使用。

考官中考官官经常会说：“单例模式了解吗？来给我手写一下！给我解释一下双重检验锁方式实现单例模式的原理呗！”
...
![img](https://pic4.zhimg.com/80/v2-0d14173c1330723b217e10219a9ab81f_720w.jpg)
...
![img](https://pic1.zhimg.com/80/v2-00c83260eb53c3f283d74b5f5031976c_720w.jpg)
...
![img](https://pic1.zhimg.com/80/v2-b6f16ec03979d78afb0d03d85bb5bb24_720w.jpg)
...
![img](https://pic3.zhimg.com/80/v2-8d4a8cfe09a58470df64c2bb88bb503a_720w.jpg)
...
![img](https://pic4.zhimg.com/80/v2-89bb4bade1ee4af723d2af9c14e42037_720w.jpg)
...
![img](https://pic2.zhimg.com/80/v2-21020d0af9f3ef0d800b5a1e38881741_720w.jpg)
...
![img](https://pic3.zhimg.com/80/v2-98848a00b03eb094ab54a4040a4e982e_720w.jpg)
...
为了让大家更清楚上面的考官题中的一些概念，我写了一个简单的线程池 Demo。
...
![img](https://pic4.zhimg.com/80/v2-6612eb191a80a7f4241c7f3de5f61213_720w.jpg)
...
![img](https://pic2.zhimg.com/80/v2-e2045a70c9d1b68726e764f96c9b4335_720w.jpg)
...
关于 Atomic 原子类这部分更多内容可以查看我的这篇文章：并发编程考官必备：[JUC 中的 Atomic 原子类总结](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s/joa-yOiTrYF67bElj8xqvg)
...
![img](https://pic1.zhimg.com/80/v2-05c47fd4c69f739a0e13e1c3c68e078c_720w.jpg)
...
> 在考官中被问到并发知识的时候，大多都会被问到“请你说一下自己对于AQS原理的理解”。下面给大家一个示例供大家参加，考官不是背题，大家一定要加入自己的思想，即使加入不了自己的思想也要保证自己能够通俗的讲出来而不是背出来。
...
![img](https://pic3.zhimg.com/80/v2-0b8b7197a68ab82003b8b0d30a28f3ee_720w.jpg)
